...
"""S5 — EV Agent RL (H-PPO) + koordinasi tak-langsung (CTDE), forecaster TETAP (Mode A).

Melatih policy H-PPO (Spesifikasi_RL_EV_LoadBalancing.md) via CONTINUING-TASK trainer
(sim persisten, potongan per-hari, trust lintas potongan; §9.1b) lalu mengevaluasinya pada
episode penuh. Forecaster non-RL = formula antrean. EstWait JUJUR = default (§13).
`--updates` = jumlah chunk-update (1 chunk = `--rollout-steps` step, default 96 = 1 hari).
"""
import argparse
import json
...
def run_s5(dataset="scenario_dataset.json", updates=540, k=3, rollout_steps=96, seed=0,
           lr=1e-4, ent_coef=0.01, alpha_wait=1.0, alpha_fair=0.3, alpha_accept=0.1,
           alpha_shape=0.5, xi=0.5, gamma=0.99, minibatch=64, epochs=4, eval_steps=None,
           honest_estwait=True):
    mode = "EstWait JUJUR (default)" if honest_estwait else "EstWait manipulatif (ablasi)"
    print(f"=== S5: EV Agent RL (H-PPO) + CTDE, forecaster tetap (Mode A) | {mode} ===")
    ...
    print(f"Melatih {updates} chunk-update (chunk={rollout_steps} step, N={tr.N} SPKLU)...")
    policy, forecaster = train_mode_a(dataset, tr, total_updates=updates)

    # Evaluasi pada episode (horizon penuh bila eval_steps=None).
    ...
    sim.export_actor_logs("s5_actor_trace")
    print("\n=== Hasil Evaluasi S5 ===")
    for kk, vv in metrics.items():
        print(f"  {kk}: {vv}")
    with open("simulation_s5_metrics.json", "w") as f:
        json.dump({"train_history": tr.history, "eval": metrics}, f, indent=2)
    with open("simulation_s5_eval_logs.json", "w") as f:
        json.dump(sim.logs, f, indent=2)
    print("\n[INFO] Disimpan ke simulation_s5_metrics.json dan simulation_s5_eval_logs.json")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--alpha-fair", type=float, default=0.3, help="bobot disparitas dalam-klaster (sekunder)")
    ap.add_argument("--alpha-accept", type=float, default=0.1)
    ap.add_argument("--alpha-shape", type=float, default=0.5, help="bobot shaping Phi (policy-invariant)")
    ...
    a = ap.parse_args()
    run_s5(a.dataset, a.updates, a.k, a.rollout_steps, a.seed, a.lr, a.ent_coef,
           a.alpha_wait, a.alpha_fair, a.alpha_accept, a.alpha_shape, a.xi, a.gamma,
           a.minibatch, a.epochs, a.eval_steps, honest_estwait=not a.manipulative)
