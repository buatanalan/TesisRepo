...
"""
Visualisasi distribusi metrik eksperimen utama 20-pass penuh.
Membaca main_experiment_results_20pass.json dan menghasilkan dashboard
multi-panel: distribusi (mean±std), train curve, radar chart, dan tabel.
"""
...
# Load data
# ─────────────────────────────────────────────
with open("main_experiment_results_20pass.json") as f:
    data = json.load(f)

comp = data["comparison"]
history = data["train_history"]  # list of dicts, length=20

SCENARIOS = ["S0_no_intervention", "S1_greedy", "S3_opsrl", "S4_marl"]
LABELS = ["S0\nNo-intervensi", "S1\nGreedy", "S3\nOP-SRL", "S4\nMARL"]
COLORS = ["#6C7A89", "#E67E22", "#27AE60", "#2980B9"]
COLORS_ALPHA = [c + "55" for c in COLORS]
...
fig.suptitle(
    "Analisis Distribusi Metrik — Eksperimen Utama 20-Pass (Lingkungan Robust)\n"
    "BETA_QUEUE_OBS=−1.2 · updates=20 · rollout=96 · k=3 · eval_seeds=5",
...
def scalar_bar(ax, key, ylabel, title, fmt=".1f", ylim=None):
    """Untuk metrik skalar (bukan dict mean/std)."""
...
means_slope = [comp[sc]["gini_slope"]["mean"] * 1e5 for sc in SCENARIOS]
stds_slope = [comp[sc]["gini_slope"]["std"] * 1e5 for sc in SCENARIOS]
x = np.arange(len(SCENARIOS))
...
for i, (m, s) in enumerate(zip(means_slope, stds_slope)):
    ax12.text(i, m - s - 0.05, f"{m:.2f}e-5", ha="center", va="top", fontsize=7, color="#E6EDF3")
...
ax_train.set_title("Kurva Training S4 MARL — 20 Chunk-Update\n(Reward, Gini, Explained Variance, KL-Divergence)",
...
# Normalize metrics: 0=worst, 1=best for each metric
# Metrics: wait(↓), herding(↓), auc_gini(↑), acceptance(↑), rec_entropy(↑), frac_inactive(↓)
metrics_radar = {
...
ax_radar.set_title("Radar: Perbandingan Multi-Metrik\n(dinormalisasi 0→1, 1=terbaik per metrik)",
                   fontsize=9, fontweight="bold", pad=18, color="#E6EDF3")
ax_radar.legend(loc="lower right", bbox_to_anchor=(1.35, -0.08), fontsize=8)

# ─── Save ────────────────────────────────────
out = "experiment_20pass_dashboard.png"
fig.savefig(out, dpi=160, bbox_inches="tight", facecolor="#0F1117")
print(f"[OK] Dashboard disimpan ke: {out}")

# ─── Print tabel ringkas ──────────────────────
print("\n" + "="*90)
print(f"{'METRIK':<30} {'S0':>12} {'S1 Greedy':>12} {'S3 OP-SRL':>12} {'S4 MARL':>12}")
print("="*90)

rows = [
    ("wait_mean (mnt)", "wait_mean", False, ".2f"),
    ("wait_p90 (mnt)", "wait_p90", False, ".1f"),
    ("frac_inactive", "frac_inactive", False, ".4f"),
    ("auc_gini (mean)", None, False, None),
    ("herding_index (mean)", None, False, None),
    ("herding_events (mean)", None, False, None),
    ("rec_entropy_norm (mean)", None, False, None),
    ("acceptance_overall (mean)", None, False, None),
    ("trust_mean", "trust_mean", False, ".4f"),
    ("trust_frac_below_0.5", "trust_frac_below_0.5", False, ".4f"),
    ("gini_final (mean)", None, False, None),
    ("gini_mean (mean±std)", None, False, None),
]

# Build table manually
table = [
    ("wait_mean (mnt)",        [comp[sc]["wait_mean"] for sc in SCENARIOS], ".2f", None),
    ("wait_p90 (mnt)",         [comp[sc]["wait_p90"] for sc in SCENARIOS], ".1f", None),
    ("frac_inactive",          [comp[sc]["frac_inactive"] for sc in SCENARIOS], ".4f", None),
    ("trust_mean",             [comp[sc]["trust_mean"] for sc in SCENARIOS], ".4f", None),
    ("trust_frac_below_0.5",   [comp[sc]["trust_frac_below_0.5"] for sc in SCENARIOS], ".4f", None),
    ("auc_gini mean",          [comp[sc]["auc_gini"]["mean"] for sc in SCENARIOS], ".4f",
                               [comp[sc]["auc_gini"]["std"] for sc in SCENARIOS]),
    ("gini_final mean",        [comp[sc]["gini_final"]["mean"] for sc in SCENARIOS], ".4f",
                               [comp[sc]["gini_final"]["std"] for sc in SCENARIOS]),
    ("gini_mean mean",         [comp[sc]["gini_mean"]["mean"] for sc in SCENARIOS], ".4f",
                               [comp[sc]["gini_mean"]["std"] for sc in SCENARIOS]),
    ("gini_slope (×1e-5)",     [comp[sc]["gini_slope"]["mean"]*1e5 for sc in SCENARIOS], ".3f",
                               [comp[sc]["gini_slope"]["std"]*1e5 for sc in SCENARIOS]),
    ("herding_index mean",     [comp[sc]["herding_index"]["mean"] for sc in SCENARIOS], ".4f",
                               [comp[sc]["herding_index"]["std"] for sc in SCENARIOS]),
    ("herding_events mean",    [comp[sc]["herding_events"]["mean"] for sc in SCENARIOS], ".1f",
                               [comp[sc]["herding_events"]["std"] for sc in SCENARIOS]),
    ("rec_entropy_norm mean",  [comp[sc]["rec_entropy_norm"]["mean"] for sc in SCENARIOS], ".4f",
                               [comp[sc]["rec_entropy_norm"]["std"] for sc in SCENARIOS]),
    ("acceptance mean",        [comp[sc]["acceptance_overall"]["mean"] for sc in SCENARIOS], ".4f",
                               [comp[sc]["acceptance_overall"]["std"] for sc in SCENARIOS]),
]

for label, means, fmt, stds in table:
    row = f"{label:<30}"
