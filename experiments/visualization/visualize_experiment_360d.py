...
"""
Visualisasi distribusi metrik eksperimen 360-pass (360 hari).
Membaca main_experiment_360d.json dan menghasilkan dashboard
...
# Load data
# ─────────────────────────────────────────────
with open("main_experiment_360d.json") as f:
    data = json.load(f)
...
fig.suptitle(
    "Analisis Distribusi Metrik — Eksperimen 360 Hari (Horizon Panjang)\n"
    "BETA_QUEUE_OBS=−1.2 · updates=30 · rollout=480 · k=3 · eval_seeds=5",
...
bar_mean_std(ax11, "gini_final", "Gini Akhir", "Gini Utilisasi (Akhir Simulasi)\n(mean±std lintas seed)",
             fmt=".3f", ylim=(0.80, 0.95))

# 1-2: gini_slope (×10⁻⁷)
ax12 = fig.add_subplot(gs[1, 2])
means_slope = [comp[sc]["gini_slope"]["mean"] * 1e7 for sc in SCENARIOS]
stds_slope = [comp[sc]["gini_slope"]["std"] * 1e7 for sc in SCENARIOS]
x = np.arange(len(SCENARIOS))
...
for i, (m, s) in enumerate(zip(means_slope, stds_slope)):
    ax12.text(i, m - s - 0.05, f"{m:.2f}e-7", ha="center", va="top", fontsize=7, color="#E6EDF3")
