...
"""Varian run_test_30d_learned_forecaster.py yang MENGHILANGKAN faktor (a) distribution-shift:
alih-alih mengumpulkan trajektori pretraining dari GreedyAgent (S1, distribusi demand beda --
least-loaded murni, tanpa herding/flocking penalty), forecaster di-pretrain dari trajektori
POLICY HPPO ITU SENDIRI (fresh/belum dilatih, tapi arsitektur & observasi identik dgn yang
dipakai saat online run 30-hari) -- via InferenceAgent membungkus tr.policy yang SAMA persis
akan dipakai di tahap online.

Dengan distribusi trafik pretraining >< online DISAMAKAN (sama-sama "kebijakan HPPO"), sisa
gap performa offline-vs-online yang masih muncul HANYA bisa dijelaskan faktor (b): forecaster
tetap buta thd EV yang sedang menuju stasiun (compute_virtual_wait dipakai sbg label, tapi
extract_features tak punya fitur traveling-EV) -- bukan lagi oleh mismatch sumber data.

Bandingkan hasil skrip ini dgn test_30d_seed0_learnedfc_summary.json (versi GreedyAgent) utk
mengisolasi kontribusi faktor (a) vs (b) pada gap offline->online forecaster.
"""
import json
...
from marl_spklu.rl.rollout import RLRolloutAgent, InferenceAgent
from marl_spklu.rl.forecaster import LearnedForecaster, FormulaForecaster, collect_forecast_dataset
...
OUT_JSONL = "test_30d_seed0_learnedfc_matched_log.jsonl"
OUT_SUMMARY = "test_30d_seed0_learnedfc_matched_summary.json"


def pretrain_forecaster_matched(policy, k, honest_estwait):
    """Sama seperti pretrain Mode B, TAPI trajektori dikumpulkan dari POLICY HPPO fresh
    (InferenceAgent), bukan GreedyAgent -- menyamakan distribusi demand dgn tahap online."""
    sim = _fresh_sim(DATASET)
    collector_agent = InferenceAgent(policy, sim, forecaster=FormulaForecaster(), k=k,
                                     honest_estwait=honest_estwait)
    X, y = collect_forecast_dataset(sim, min(PRETRAIN_STEPS, sim.max_steps), agent=collector_agent)
    ...
    err = pred - y
    fit_stats = {
        "collector": "InferenceAgent(fresh HPPO policy) -- MATCHED ke distribusi online",
...
def main():
    # Bangun trainer/policy DULU (fresh, seed=0) -- policy yang SAMA dipakai baik utk
    # koleksi data pretraining maupun tahap online 30-hari (tak ada gradient step di antaranya,
    # jadi bobotnya identik saat online run mulai -- reproduksi run sebelumnya persis, hanya
    # sumber data pretraining forecaster yang beda).
    tr = TorchContinuingTrainer(DATASET, k=3, rollout_steps=CHUNK, seed=SEED,
                                verbose=False, honest_estwait=True)

    print("=== Tahap 1: pretrain LearnedForecaster offline, trajektori DISAMAKAN (fresh HPPO policy) ===")
    forecaster, fit_stats = pretrain_forecaster_matched(tr.policy, tr.k, tr.honest_estwait)
    print("Fit forecaster (in-sample, matched):", json.dumps(fit_stats, indent=2))
    ...
    with open(OUT_JSONL, "w") as flog:
        flog.write(json.dumps({"event": "config", "seed": SEED, "dataset": DATASET,
                               "forecaster": "LearnedForecaster(Mode B, MATCHED collector, frozen)",
    ...
    for day in sorted(by_day_sessions):
        ...
        fc_daily.append({"day": day, "n": len(recs), "mae": float(np.mean(np.abs(err))),
                         "bias_pred_minus_act": float(np.mean(err)), "corr": corr})
    ...
    print("\n=== 3. PERFORMA FORECASTER (matched collector) ===")
