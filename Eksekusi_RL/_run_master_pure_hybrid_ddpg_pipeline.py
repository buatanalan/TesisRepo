"""Pelatihan Master-Hybrid DDPG (2026-08-29) -- MASTER murni + modul P (late-inject,
opsi 'b') + station attention DI SISI AKTOR + keluaran VEKTOR->bid skalar kontinu
(argmax menang, mekanisme ASLI MASTER, TAK dipaksa softmax -- lih. `master_pure_
hybrid_policy.py`). Kelas aktor BARU (`MasterHybridDDPGActor`), tapi trainer REUSE
`MasterPureTrainer` (`master_pure_trainer.py`) via parameter `actor_cls`/`actor_kwargs`
BARU (tambahan aditif, bukan menumpuk) -- kritik/replay/delayed-access TAK berubah.

Tiga tahap sama pola `_run_master_pure_pipeline.py`:
  1. `--mode pretrain_specialist --stream-select 0` (wait)
  2. `--mode pretrain_specialist --stream-select 1` (gini)
  3. `--mode dgr`

Jalankan (server, latar belakang):
    nohup .venv/Scripts/python.exe Eksekusi_RL/_run_master_pure_hybrid_ddpg_pipeline.py \
        --mode pretrain_specialist --stream-select 0 \
        > Eksekusi_RL/outputs/master_pure_hybrid_ddpg_pipeline.log 2>&1 &
"""
...
def train_one(seed):
    ...
    if args.mode == "pretrain_specialist":
        ...
        n_spklu = len(_fresh_sim(DATASET).spklus)
        tag0 = _specialist_tag(0, args.specialist0_tag)
        tag1 = _specialist_tag(1, args.specialist1_tag)
        kw["specialists"] = [_load_specialist(tag0, args.specialist_seed, n_spklu),
                             _load_specialist(tag1, args.specialist_seed, n_spklu)]
...
print(f"[{elapsed()}] === PELATIHAN Master-Hybrid DDPG ({args.n_train_seed} seed) ===", flush=True)
results = []
