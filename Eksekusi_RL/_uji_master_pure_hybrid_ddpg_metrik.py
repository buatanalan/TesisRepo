"""Evaluasi Master-Hybrid DDPG dgn metrik kaya sama `_uji_konsolidasi.py`.

Pemakaian:
    python _uji_master_pure_hybrid_ddpg_metrik.py 0,1,2 30d master_hybrid_ddpg_dgr
"""
import sys, os
...
from marl_spklu.rl.master_pure_hybrid_policy import MasterHybridDDPGActor, MasterHybridDDPGInferenceAgent
...
def muat_policy(seed, n_spklu):
    ...
    assert os.path.exists(ckpt), (
        ...
        f"latih dulu lewat: python _run_master_pure_hybrid_ddpg_pipeline.py --n-train-seed {seed+1} ...")
    pol = MasterHybridDDPGActor(n_spklu, **ACTOR_KW)
...
def fac_dari_policy(policy):
    def fac(sim, _pol=policy):
        agent = MasterHybridDDPGInferenceAgent(_pol, forecaster=FormulaForecaster(), k=K_REC)
