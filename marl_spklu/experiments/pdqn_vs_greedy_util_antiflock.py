"""Uji PDQN (+ suku ANTI-OVERSHOOT) vs Greedy-util -- opsi #1 dari diskusi
...
from marl_spklu.experiments.pdqn_baseline import PDQN_REWARD_ANTIFLOCK, train_pdqn
from marl_spklu.rl.pdqn_agent import PDQNInferenceAgent

DATASET = "scenario_dataset_7d.json"
STOCHASTIC = True
WILLINGNESS_RATIO = 5.0   # default longgar -- sama dgn uji rigor awal (bukan opsi #2)
TRAIN_SEEDS = (0, 1, 2)
EVAL_SEEDS = range(10)
...
def main():
    ...
    for mu in (0.2, 0.5, 0.8):
        ...
        for ts in TRAIN_SEEDS:
            ...
            q_net, _ = train_pdqn(DATASET, mu, n_updates=args.n_updates,
                                  ...
                                  reward_calc=PDQN_REWARD_ANTIFLOCK,
                                  willingness_ratio=WILLINGNESS_RATIO)
        ...
        greedy_better = bool(diff.mean() < 0 and pval < 0.05)
        verdict = ("PDQN SIGNIFIKAN lebih baik" if pdqn_better else
                  "Greedy-util SIGNIFIKAN lebih baik" if greedy_better else "TIDAK signifikan")
        results[mu] = {
            ...
            "pdqn_better": pdqn_better, "greedy_better": greedy_better, "verdict": verdict,
        }
        print(f"\nSelisih (Greedy-util - PDQN) rata2 = {diff.mean():+.4f} +- {diff.std():.4f}")
        print(f"Wilcoxon signed-rank: statistic={stat:.2f} p={pval:.4f} -> {verdict}")
    ...
    for mu, r in results.items():
        print(f"mu={mu}: diff={r['diff_mean']:+.4f}  p={r['wilcoxon_p']:.4f}  -> {r['verdict']}")
